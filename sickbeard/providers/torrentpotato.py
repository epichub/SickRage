...
class TorrentPotatoProvider(TorrentProvider):

    def __init__(self):

        TorrentProvider.__init__(self, "TorrentPotato")
    ...
    def search(self, search_strings, age=0, ep_obj=None):  # pylint: disable=too-many-branches, too-many-locals
        ...
        if self.passkey:
            search_params["passkey"] = self.passkey

        if self.username:
            search_params["user"] = self.username
        ...
        for mode in search_strings:
            ...
            for search_string in search_strings[mode]:
                ...
                #http://localhost:9117/api/v2.0/indexers/kickasstorrent-kathow/results/potato/
                search_url = urljoin(url, "")
                ...
                for result in parsed_json.pop("results", {}):
                    try:
                        title = result.pop("release_name", "")

                        info_hash = result.pop("infoHash", "")
                        download_url = result.pop("download_url", "")
...
provider = TorrentPotatoProvider()
